...
sum(case when m.date_published is null then 1 else 0 END) as date_published_null,
...
sum(case when m.worlwide_gross_income is null then 1 else 0 END) as WGI_null,
sum(case when m.languages is null then 1 else 0 END) as LANG_NULL,
sum(case when production_company is null then 1 else 0 END) as PC_null
from movie as m;


-- Country, Worlwide_gross_income , Languages and production has null values in it
...
-- The highest number of movies are being sold in March month and lowest is in December.
...
-- Type your code below:
select count(*) as Number_of_movies, year from movie where year='2019' and (country like '%INDIA%' or country like '%USA%') ;


/* USA and India produced more than a thousand movies i.e 1059(you know the exact number!) in the year 2019.
...
with movie_rank_on_avg_rating as 
(select m.title,
		r.avg_rating,
...
inner join movie m
on r.movie_id=m.id)
select * from movie_rank_on_avg_rating
where movie_rank<=10;
...
on m.id=r.movie_id
where m.country like '%USA%' and m.year='2017' and r.total_votes>1000
group by genre 
...
-- Type your code below:
select r.median_rating,count(m.id) as movie_released
from movie m inner join ratings r
on m.id=r.movie_id
...
-- Answer is Yes, German movies has 106710 votes, on the other side Italian movies only have 77965 votes
...
-- Type your code below:
with top_genre as(
select g.genre,count(movie_id) as Num_Movies
from  genre g
inner join ratings r
using(movie_id)
where avg_rating >8
